...
//767--Nucleo F767ZI    429--F429I_DISCO    407---M4_DEMO    103---BLUEPILL   105---Aseko LCD   405---Aseko LCD
#if defined(STM32F1)
  #define STM32_CPU     103
#elif defined(STM32F2)
  #define STM32_CPU     205
#elif defined(STM32F4)
  #define STM32_CPU     405
#else
  #error "Choose MCU type"      
  //#define STM32_CPU   429?     
#endif
...
#define ORIENTACION     4  // 0, 1, 2, 3, FT81X 
...
// Define video player                     
#if(STM32_CPU == 405)
#define VIDEO_BUFF      32768  // ZMENA BUFF
#else
#define VIDEO_BUFF      4096  // ZMENA BUFF
#endif

// Define speed of SD
#define SPI_SD_MHZ      32
...
uint32_t loadptr;
...
#if(STM32_CPU == 405)  
  byte loadMcuFlash(void (*progress)(long, long) = NULL);
#endif
  byte load(const char *filename, void (*progress)(long, long) = NULL);
#if(STM32_CPU == 405)  
  byte load_from_flash(void (*progress)(long, long) = NULL);            // ZMENA FLASH
#endif
  ...
  uint32_t storage_GetCardSizeKB(void); // ZMENA
  uint32_t storage_GetCardFreeKB(void);
  uint32_t storage_GetCardUsedKB(void);
  uint8_t storage_GetFatType(void);
  uint8_t storage_GetCardType(void);
  uint8_t storage_GetCID(void);
  uint8_t storage_GetVersionInt(void);
  uint8_t storage_GetVersionDec(void);
  uint8_t storage_GetMonth(void);
  uint16_t storage_GetYear(void);
  uint32_t storage_GetSN(void);
  uint8_t storage_GetCRC7(void);
  char* storage_GetProduct(void);
  void storage_PrintListOfFiles(print_t *);
  uint32_t storage_GetNumberOfFiles(void);
  uint32_t storage_GetNumberOfDirectories(void);
  uint32_t storage_ResumeCard(void);
  uint32_t storage_CardExist(void);
...
class Streamer {
public:
  int begin(const char *rawsamples,
             uint16_t freq = 44100,   //8000,  // 44100
             byte format =  ADPCM_SAMPLES, //LINEAR_SAMPLES, // ULAW_SAMPLES,  // ADPCM_SAMPLES
             uint32_t _base = (/*0x40000UL*/ 0xF4240 - 8192), uint16_t size = 8192) {
    GD.__end();
...
//Serial.println("Streamer open ok");
      ...
      GD.wr(REG_GPIO_DIR, 0x01 | GD.rd(REG_GPIO_DIR));  // ZMENA
      GD.wr(REG_GPIO, 0x01 | GD.rd(REG_GPIO));          // ZMENA
...
//Serial.println(" n=" + String(n));
    ...
    if(Serial1.available() || Serial.available()) p = m;
    if(p >= m)
    {
      GD.wr(REG_GPIO_DIR, ~0x01 & GD.rd(REG_GPIO_DIR));  // ZMENA
      GD.wr(REG_GPIO, ~0x01 & GD.rd(REG_GPIO));          // ZMENA
...
class MoviePlayer
{
  ...
  File r;
  int loadsector() {
    byte buf[VIDEO_BUFF]; // ZMENA BUFF 
    GD.__end();   
    //r.readsector(buf);
    int32_t c = r.curPosition();
    int32_t p = r.peek();
    int n = r.read(buf,VIDEO_BUFF); // POKUS BUFFER
    
    if (Serial1.available() > 0) return -1;  // ZMENA
   
    GD.resume(); 
    GD.wr_n(mf_base + wp, buf, VIDEO_BUFF); // POKUS BUFFER   
    
    wp = (wp + VIDEO_BUFF) & (mf_size - 1); // ZMENA BUFF  
  }
...
//Serial.println("MoviePlayer begin filanem=" + String(filename));
    ...
    while (wp < (mf_size - VIDEO_BUFF)) { // ZMENA BUFF
      loadsector();
    }
   ...
   int service() {
    ...
    if (r.peek() == -1) {
      ...
      //if(Serial1.available() > 0) fullness = mf_size - VIDEO_BUFF;
      if(Serial1.available() > 0) return 0;
      while (fullness < (mf_size - VIDEO_BUFF)) {
        loadsector();
        fullness += VIDEO_BUFF;
  ...
  void play() {                      
    //GD.cmd_regwrite(REG_GPIO_DIR, 0x81);           // ZMENA
    //uint32_t gpio_tmp;                             // ZMENA
    //GD.cmd_regread(gpio_tmp);                      // ZMENA
    //GD.cmd_regwrite(REG_GPIO, gpio_tmp | 0x81);    // ZMENA
...
//    GD.cmd_playvideo(OPT_MEDIAFIFO | OPT_FULLSCREEN | OPT_SOUND);  // ADD OPT_SOUND by @lightcalamar
    ...
    GD.cmd_playvideo(OPT_MEDIAFIFO | OPT_SOUND | OPT_FULLSCREEN | OPT_NOTEAR);  // ADD OPT_SOUND by @lightcalamar 11/06/2018 // ZMENA
    
    // Delete OPT_FULLSCREEN
    GD.flush();
    while (service())
    {
      IWDG->KR = (uint16_t) 0xAAAA; // ZMENA
      if ((Serial1.available() || Serial.available()) > 0) break;
    }
    
    GD.cmd_memcpy(0, 0, 4);
    
    if ((Serial1.available() == 0) && (Serial.available() == 0)) GD.finish();
    
    //GD.cmd_regwrite(REG_GPIO_DIR, 0x80);           // ZMENA
