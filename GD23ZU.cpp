...
#include "transports/wiring.h"
  ...
  #if(STM32_CPU == 103) // ZMENA
   //SdFat SD(2);       // M4DEMO with PB12
   SdFatEX SD(2);       // ZMENA
  #endif 
  
  #if(STM32_CPU == 105) // ZMENA
   SdFatSdio SD;       // ZMENA
  #endif  
  
  #if(STM32_CPU == 205) // ZMENA
   SdFatSdio SD;       // ZMENA
  #endif   
  
  #if(STM32_CPU == 405) // ZMENA
   SdFatSdio SD;       // ZMENA
  #endif  
...
byte ft8xx_model = 0x1;     // 0:FT800 / 1:FT81x // ZMENA
uint16_t FT800_model = 0x13; // 0x00:FT800/0x10:FT810/0x11:FT811/0x12:FT812/0x13:FT813  // ZMENA
...
//RndMnkIII
#if(STM32_CPU == 405)
const uint16_t GDClass::TAM_BUFFER_SD=32768; // ZMENA
#else
const uint16_t GDClass::TAM_BUFFER_SD=4096; // ZMENA
#endif
...
void GDClass::begin(uint8_t options) {
 ...
 if (options & GD_STORAGE)
  {
    ...
    #if defined(ARDUINO_ARCH_STM32) && (STM32_CPU != 405) && (STM32_CPU != 105)
      //SD.begin(SD_PIN, SD_SCK_MHZ(36));      // STM32
      //SD.begin(SD_PIN, SD_SCK_MHZ(SPI_SD_MHZ));      // STM32F103 // ZMENA // TEST
      uint8_t attempts = 5;
      while(attempts--)
      {
        if(SD.begin(SD_PIN, SD_SCK_MHZ(SPI_SD_MHZ)) == 1) break;
        delay(100);
      }
    #endif
 ...
 #if(STM32_CPU != 103)
 GDTR.wr32(REG_PWM_HZ, 8000);
 #else
 GDTR.wr32(REG_PWM_HZ, 1500);
 #endif
 ...
 GDTR.hostcmd(0x00); // switch to active mode
 GDTR.hostcmd(0x61); // switch to active mode
...
if (SizeFT813==7)
  {
     ...
     GD.wr32(REG_HSIZE, 800);
     //GD.wr32(REG_HSYNC0, 0);
     GD.wr32(REG_HSYNC0, 210); // ZMENA ZRNI
     //GD.wr32(REG_HSYNC1, 20);
     GD.wr32(REG_HSYNC1, 210+10); // ZMENA ZRNI
	
     GD.wr32(REG_VSIZE, 480);
     GD.wr32(REG_VCYCLE, 525);
     GD.wr32(REG_VOFFSET, 23);
     //GD.wr32(REG_VSYNC0, 0);
     GD.wr32(REG_VSYNC0, 22); // ZMENA ZRNI
     //GD.wr32(REG_VSYNC1, 10);
     GD.wr32(REG_VSYNC1, 22+10); // ZMENA ZRNI
	
     GD.wr32(REG_SWIZZLE, 3);//3 for GD2
	
     GD.wr32(REG_PCLK, 2);
     GD.wr32(REG_PCLK_POL, 1);
     //GD.wr32(REG_CSPREAD, 1);
     GD.wr32(REG_CSPREAD, 0); // ZMENA ZRNI
     GD.wr32(REG_DITHER, 0);  // ZMENA vychozi stav
     GD.wr32(REG_OUTBITS, 0x0);  // ZMENA vychozi stav
/*
...
#define DispDither     1
  ...
  GD.wr32(REG_HCYCLE, DispHCycle);
  GD.wr32(REG_HOFFSET, DispHOffset);
  GD.wr32(REG_HSIZE, DispWidth);
  GD.wr32(REG_HSYNC0, DispHSync0);
  GD.wr32(REG_HSYNC1, DispHSync1);
  
  GD.wr32(REG_VCYCLE, DispVCycle);
  GD.wr32(REG_VOFFSET, DispVOffset);
  GD.wr32(REG_VSIZE, DispHeight);
  GD.wr32(REG_VSYNC0, DispVSync0);
  GD.wr32(REG_VSYNC1, DispVSync1);
  
  GD.wr32(REG_SWIZZLE, DispSwizzle);
  GD.wr32(REG_CSPREAD, DispCSpread);
  GD.wr32(REG_DITHER, DispDither);
  GD.wr32(REG_PCLK_POL, DispPCLKPol);
  GD.wr32(REG_PCLK, DispPCLK);
  */
}
...
//  if (CALIBRATION & (calibrate != 0)) {
  if (calibrate != 0) {
    //uint16_t Status;
    uint8_t Data;                       // ZMENA FLASH
    //Status = EEPROM.read(0x10,&Data);  // ZMENA FLASH
    Data = EEPROM.read(0x10);            // ZMENA FLASH
    if (Data != 0x7c || calibrate == 2) {
      self_calibrate();
      EEPROM.format();                   // ZMENA FLASH
      for (int i = 0; i < 24; i++) { 
		    Data = GDTR.rd(REG_TOUCH_TRANSFORM_A + i);
        EEPROM.write(0x20 + i, Data);  // ZMENA FLASH
      }
      EEPROM.write(0x10, 0x7c);  // is written!  // ZMENA FLASH
    } else {
      for (int i = 0; i < 24; i++) {
		    Data=EEPROM.read(0x20+i);     // ZMENA FLASH
...
uint32_t GDClass::storage_GetCardSizeKB(void) {
  //Sd2Card volume;
  //return volume.cardSize();
  IWDG->KR = (uint16_t) 0xAAAA;
  return (SD.card()->cardSize()/2);
}

uint32_t GDClass::storage_GetCardFreeKB(void) {
  //return (SD.card()->cardSize() / 2 - SD.vwd()->fileSize() / 512);
  IWDG->KR = (uint16_t) 0xAAAA;
  return (SD.vol()->freeClusterCount() * (SD.vol()->blocksPerCluster()/2));   
}

uint32_t GDClass::storage_GetCardUsedKB(void) {
  //Sd2Card volume;
  //return volume.cardSize();
  IWDG->KR = (uint16_t) 0xAAAA;    
  return (GD.storage_GetCardSizeKB()-GD.storage_GetCardFreeKB());
}

uint8_t GDClass::storage_GetFatType(void) {
  IWDG->KR = (uint16_t) 0xAAAA;
  return SD.fatType();
}

uint8_t GDClass::storage_GetCardType(void) {
  IWDG->KR = (uint16_t) 0xAAAA;
#if(STM32_CPU == 405)
  return 0; //SD.card()->type(); // ZMENA TODO
#else
  return (SD.card()->type());
#endif
}

uint8_t GDClass::storage_GetCID(void) {
  IWDG->KR = (uint16_t) 0xAAAA;
...
uint8_t GDClass::storage_GetVersionInt(void) {
  IWDG->KR = (uint16_t) 0xAAAA;
...
uint8_t GDClass::storage_GetVersionDec(void) {
  IWDG->KR = (uint16_t) 0xAAAA;
...
uint8_t GDClass::storage_GetMonth(void) {
  IWDG->KR = (uint16_t) 0xAAAA;
  
  cid_t cid;
  SD.card()->readCID(&cid);
  
  return (cid.mdt_month);
}

uint16_t GDClass::storage_GetYear(void) {
  IWDG->KR = (uint16_t) 0xAAAA;
  
  cid_t cid;
  SD.card()->readCID(&cid);
  
  return (2000 + cid.mdt_year_low + 10 * cid.mdt_year_high);
}

uint32_t GDClass::storage_GetSN(void) {
  IWDG->KR = (uint16_t) 0xAAAA;
...
uint8_t GDClass::storage_GetCRC7(void) {
  IWDG->KR = (uint16_t) 0xAAAA;
...
char* GDClass::storage_GetProduct(void) {
  IWDG->KR = (uint16_t) 0xAAAA;
...
void GDClass::storage_PrintListOfFiles(print_t *ptr) {
  IWDG->KR = (uint16_t) 0xAAAA;
  SD.ls(ptr, "/", LS_R | LS_DATE | LS_SIZE);
}

uint32_t GDClass::storage_GetNumberOfFiles(void) {
  SdFile dirFile;
  SdFile file;
  
  uint32_t n_files = 0;
  
  if (!dirFile.open("/", O_READ)) return 0;
    
  while (file.openNext(&dirFile, O_READ)) {
    IWDG->KR = (uint16_t) 0xAAAA;
    if (file.fileSize() > 0) n_files++;
    file.close();
  }
...
uint32_t GDClass::storage_GetNumberOfDirectories(void) {
  SdFile dirFile;
  SdFile file;
  
  uint32_t n_directories = 0;
  
  if (!dirFile.open("/", O_READ)) return 0;
    
  while (file.openNext(&dirFile, O_READ)) {
    IWDG->KR = (uint16_t) 0xAAAA;
    if (file.fileSize() == 0) n_directories++;
    file.close();
  }
...
uint32_t GDClass::storage_ResumeCard(void) {
  ...
  #if defined(ARDUINO_ARCH_STM32) && (STM32_CPU != 405) && (STM32_CPU != 105)
    //SD.begin(SD_PIN, SD_SCK_MHZ(36));      // STM32
    ret = SD.begin(SD_PIN, SD_SCK_MHZ(SPI_SD_MHZ));      // STM32F103 // ZMENA // TEST
  #endif
  ...
  #if defined(TEENSYDUINO) 
    ret = SD.begin();                            // Teensy 3.6-3.5
  #endif
  
  return ret;
}

uint32_t GDClass::storage_CardExist(void) {
  //return SD.cardErrorCode();
  
  return SD.exists("/");
}

void GDClass::eeprom_format(void) {
  EEPROM.format();
}
...
void GDClass::self_calibrate(void) {
  cmd_dlstart();
  Clear();
  cmd_regwrite(REG_PWM_DUTY, 0xFF);
  cmd_text(GD.w / 2, GD.h / 2, 31, OPT_CENTERX, "please tap on the dot");
  cmd_calibrate();
  finish();
  cmd_loadidentity();
  cmd_dlstart();
...
byte GDClass::load(const char *filename, FUNC_POINTER progress)
{
  ...
  //archivo = SD.open(filename, O_READ);  // TODO SD  
...
#if(STM32_CPU == 405)
byte GDClass::load_from_flash(/*const char *filename, */FUNC_POINTER progress)
{
  return (loadMcuFlash(progress)); 
}
#endif
...
#if(STM32_CPU == 405)
byte GDClass::loadMcuFlash(FUNC_POINTER progress)
{
  ...
  int tamArchivo=sizeof(aseko_image); 

    while( offset < tamArchivo)
    {
        uint16_t m = ( ((tamArchivo-offset) > TAM_BUFFER_SD) ? TAM_BUFFER_SD : (tamArchivo-offset));
        //archivo.read(GD.buf,m);
        memcpy(GD.buf, aseko_image+offset, m );
        ...
        while (offsetFT < m) 
        {
              ...
              if (progress)
                (*progress)((offset+offsetFT), tamArchivo);
#ifdef DUMMY_GDCOPYRAM
                memcpy (FTbuf, GD.buf+offsetFT, wbn );
#else
                GD.copyram((GD.buf+offsetFT), wbn);
#endif              
              offsetFT+=n;
              bytesDisponibles-=n;
              GDTR.stop();
